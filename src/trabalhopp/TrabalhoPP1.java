...
public class TrabalhoPP1 {
    ...
    public static void main(String[] args) throws UnidadeTerritorialException, RegistoSalarioException {
        ...
        UnidadeTerritorial[] vetorCasted1 = GestaoUnidadeTerritorial.castToUnidadeTerritorial(gut.getUTsPorTipo(nuts1));
        UnidadeTerritorial[] vetorCasted2 = GestaoUnidadeTerritorial.castToUnidadeTerritorial(gut.getUTsPorTipo(nuts2));
        UnidadeTerritorial[] both;
        both = GestaoUnidadeTerritorial.concat(vetorCasted1, vetorCasted2);
        //gutMunicipios - vetor municipios 
        GestaoUnidadeTerritorial gutNuts = new GestaoUnidadeTerritorial(both);
        gestorSalario gestorsalariosFiltrados = new gestorSalario();

        int i = 0;

        for (UnidadeTerritorial ut : both) {
            UnidadeTerritorial utTemp = (UnidadeTerritorial) gutNuts.getUT(i);
            i++;
            gestorSalario gestor_sal = (gestorSalario) utTemp.getGestaoSal();
            Salario[] salariosCasted = gestorSalario.castToSalarios(gestor_sal.getSalarios(generos1, anos1, nuts1));
            gestorsalariosFiltrados.addRegistoSalario(salariosCasted);
        }
        Salario[] salariosFiltrados = gestorsalariosFiltrados.getSalarios();
        ...
        for (int z = 0; z < salariosFiltrados.length; z++) {
            Salario tempSalario = salariosFiltrados[z];
            UnidadeTerritorial utTemp = (UnidadeTerritorial) tempSalario.getUt();
            try {
                Localizacao lTemp = new Localizacao(utTemp.getNome() + ",Portugal", Language.PT);
                MyMarker mTemp = new MyMarker(utTemp.getNome(), lTemp.getCoordinate().getLatitude(), lTemp.getCoordinate().getLongitude(), utTemp.getTipoUt()/*utTemp.getGestaoSal().SalariosToString()*/);
                map.addMarker(mTemp);
