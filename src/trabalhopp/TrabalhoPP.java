...
import Classes.Mapeamento.GestaoMapeamento;
...
public class TrabalhoPP {
    ...
    public static void main(String[] args) throws UnidadeTerritorialException, RegistoSalarioException, IOException, CoordinatesNotFound {
        ...
        GestaoMapeamento mapsContainer = new GestaoMapeamento(m1.getMapeamentos());
        Mapeamento mapAmarante=mapsContainer.getMapByUT(gut.getUtporNome("Amarante"));
        System.out.println(mapAmarante.toString());
        ...
        for (int i = 0; i < 10; i++) {
            ...
            try {
                ...
                MyMarker mTemp = new MyMarker(utTemp.getNome(), lTemp.getCoordinate().getLatitude(), lTemp.getCoordinate().getLongitude(), utTemp.getNome()/*utTemp.getGestaoSal().SalariosToString()*/);
                map.addMarker(mTemp);
